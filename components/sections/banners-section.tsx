...
export function BannersSection() {
	...
	return (
		<PosterGrid className="before:pointer-events-none before:absolute before:left-0 before:top-0 before:z-10 before:h-full before:w-full before:bg-[url('https://www.ui-layouts.com/noise.gif')] before:opacity-[0.05] before:content-['']" id="banners" extraGuides={[{ type: "horizontal", position: "75%" }]}>
			<div className="relative col-span-4 row-span-3 overflow-hidden max-[1024px]:col-span-2">
...
<div ref={carouselRef} className="h-full overflow-hidden scroll-smooth">
	...
	<div
		key={banner.image}
		className="relative h-full min-w-0 flex-[0_0_25%] pr-[var(--poster-gap-x)] max-[1024px]:flex-[0_0_50%]"
	>
		<button
			type="button"
		...
		>
			...
			<p className="col-span-2 col-start-2 row-start-4 self-center text-center text-[1.27vw] font-medium uppercase max-[1024px]:col-start-1 max-[1024px]:text-[clamp(0.9rem,2vw,1.2rem)]">
			...
			<h2 className="col-start-1 col-span-1 row-start-4 self-end text-[1vw] font-black uppercase max-[1024px]:text-[clamp(0.9rem,2vw,1.2rem)]">
				БАННЕРЫ
			</h2>
		</PosterGrid>
	)
